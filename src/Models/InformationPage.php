...
class InformationPage extends Model
{
  ...
  {
    return $this->hasMany( InformationPageArticle::class, 'information_page_id' );
  }
  
  public function displayArticles()
  {
    return $this->hasMany( InformationPageArticle::class, 'information_page_id' )->where( function ($query) {
  ...
  {
    return $this->hasMany( InformationPageBanner::class, 'information_page_id' );
  }
  
  public function displayBanners()
  {
    return $this->hasMany( InformationPageBanner::class, 'information_page_id' )->where( function ($query) {
        $query->where('status', 'PUBLISHED')->orWhere('status', 'SCHEDULED')
              ->where('status_date', '>=', now());
      }
    )->orderBy('order', 'asc');
